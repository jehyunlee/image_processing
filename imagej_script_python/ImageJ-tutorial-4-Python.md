...
### 4.6. `try`, `except`, `finally` : 예외 처리

* 프로그램 구동 중 발생하는 예외 경우를 처리하기 위한 명령어입니다.
* `try`에 있는 명령을 먼저 실행하고, 에러와 같은 예외 상황이 발생하면 `except`에 있는 내용이 실행됩니다. 그리고 예외 상황 발생 여부와 관계없이 `finally`가 실행됩니다.
...
* 사전에 정의된 [발생오류 1, 2]와 발생오류명이 일치하지 않을 경우 `except`절로 넘어가지 않습니다.
  ```python
  try:
      실행을 시도할 명령
  except 발생오류 1:
